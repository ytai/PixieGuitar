...
/* 15.10.2013 Martin Hollenweger
                 Added support for dsPIC33E using Microchip XC16 V1.11
                 Limits:
...
void __attribute__((naked)) vPortYield() {
  portSAVE_CONTEXT();
  vTaskSwitchContext();
  portRESTORE_CONTEXT();
  asm volatile("retfie");
...
uint32_t xPortGetRunTimeCounterValue( void ) {
  unsigned ipl = _IPL;
  _IPL = configKERNEL_INTERRUPT_PRIORITY;
  uint16_t t1 = TMR1;
  uint32_t ticks = tick_count;
  _IPL = ipl;

  uint32_t us = (uint32_t) ticks * (1000000 / configTICK_RATE_HZ) +
                (uint32_t) t1 * portTIMER_PRESCALE / (configCPU_CLOCK_HZ / 1000000);
  return us;
}
